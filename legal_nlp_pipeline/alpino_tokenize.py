...
def alpino_tokenize_text_file(text_file_path: Path, target_dir_path: Path,
                              out_suffix: str):
    from logging import debug, info, warning
    from subprocess import check_call, DEVNULL

    if text_file_path.is_file() and text_file_path.stat().st_size != 0:
        tokenized_file_path = target_dir_path.joinpath(
            text_file_path.name).with_suffix(out_suffix)
        if not tokenized_file_path.is_file() or tokenized_file_path.stat(
        ).st_size == 0:
            command = (ALPINO_TOKENIZE_EXECUTABLE, )
            with text_file_path.open(mode='rb') as text_file:
                with tokenized_file_path.open(
                        mode='wb') as tokenized_text_file:
                    check_call(
                        command,
                        stdin=text_file,
                        stdout=tokenized_text_file,
                        env=ALPINO_ENV,
                        cwd=ALPINO_HOME)  # stderr=DEVNULL
                    info("Tokenized to '{tokenized_file_path}'. ".format(
...
def alpino_tokenize_text_files(extracted_dir_path: Path, target_dir_path: Path,
    ...
    text_files_paths = (
        extracted_dir_path.joinpath(ecli).with_suffix(in_suffix)
