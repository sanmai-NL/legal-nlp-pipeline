...
#            IGNORECASE)
RULES = compile(
    pattern=r'(:?[_\-–—=]{3,}(:?\s?[_\-–—=])*)+')  # Horizontal text rules TODO: add newline anchors?
LISTS = compile(
    pattern=r'((?<=[:;\.])\s+(?:(?:\(?[a-z]\))|[•\-\*])[^;]+)',
    flags=MULTILINE)
    ...
    for xml_ruling_file_path in xml_ruling_files:
        if xml_ruling_file_path.is_file() and xml_ruling_file_path.stat(
            ...
            if not text_file_path.is_file() or text_file_path.stat(
                ...
                info(
                    "Extracted wraking text to '{text_file_path}' => txt: {status:s}".format(
                        text_file_path=text_file_path, status=status))
        else:
            raise RuntimeError(
                "XML ruling file '{xml_ruling_file_path}' has size zero or does not exist. ".format(
