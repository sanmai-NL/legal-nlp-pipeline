...
def select_tokenized_files(tokenized_files_dir_path: Path, target_dir_path: Path, work_distribution):
    from logging import info

    tokenized_files = (tokenized_files_dir_path.joinpath(ecli + '.xml.txt.tok') for ecli in work_distribution)
...
def determine_zittingsplaats(ruling_tree):
    ...
    xpaths = (XPath('/open-rechtspraak/rdf:RDF/rdf:Description/dcterms:spatial[@rdfs:label]/text()',
...
def extract_uitspraak(ruling_tree):
    ...
    xpath_strs = ["/open-rechtspraak/rvr:uitspraak/rvr:section//rvr:*/text()[contains(., 'DE UITSPRAAK')]/"
    ...
    if len(items) >= 1:
        if len(items) <= 3:
            return filter_out_wijzers(clean(items[0]))  # TODO: if != '' ; precedence of XPaths
        elif len(items) > 3:
            assert False
    else:
        return None
...
def extract_standpunt_ovj(ruling_tree):
    ...
    xpath_strs = ("/open-rechtspraak/rvr:uitspraak/rvr:section//rvr:parablock/rvr:para/"
...
def extract_composite_ruling(ruling_tree):
    tenlastelegging = extract_single_tenlastelegging(extract_tenlastelegging(ruling_tree))
...
def extract_composite_rulings(xml_ruling_files, target_dir_path: Path):
    ...
    for ruling_file_path in xml_ruling_files:
        ...
        tenlastelegging, standpunt_adv, standpunt_ovj, beslissing = extract_composite_ruling(ruling_tree)
        ...
        if tenlastelegging is not None and standpunt_adv is not None and \
            ...
            extraction_status = "tlg: {0}  sadv: {1}  sovj: {2}  bs: {3}".format(
                'OK' if tenlastelegging is not None else '--',
                'OK' if standpunt_adv is not None else '--',
                'OK' if standpunt_ovj is not None else '--',
                'OK' if beslissing is not None else '--')
            info(extraction_status)
            if tenlastelegging is not None and extraction_status.count("OK") == 3:
                almost_count += 1
                # print(1)
