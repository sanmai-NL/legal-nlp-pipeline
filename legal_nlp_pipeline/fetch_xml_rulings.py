...
NAMESPACE_PREFIX_MAP = {'atom': 'http://www.w3.org/2005/Atom',
                        ...
                        'bwb': 'bwb-dl',
                        'cvdr':
                        'http://decentrale.regelgeving.overheid.nl/cvdr/',
                        'psi': 'http://psi.rechtspraak.nl/',
                        'rdfs': 'http://www.w3.org/2000/01/rdf-schema#',
                        'rvr':
                        'http://www.rechtspraak.nl/schema/rechtspraak-1.0'}
...
def process_eclis(relevant_eclis: Iterable, ruling_filter_xpath_spec: str,
    ...
    for ecli in relevant_eclis:
        try:
            ...
            if not ecli_file_path.is_file() or ecli_file_path.stat(
            ).st_size == 0:  # TODO: seems not to work??
...
def process_ecli_batch(xml_rulings_dir_path: Path, data_dir_path: Path,
    ...
    ecli_batch_file_path = xml_rulings_dir_path.joinpath(complete_query +
                                                         '.xml')
    if not ecli_batch_file_path.is_file() or ecli_batch_file_path.stat(
    ).st_size == 0:
        url = ZOEKEN_URL_PREFIX + complete_query

        try:
            ecli_batch_tree = fetch(
                url=url, expected_content_type=EXPECTED_CONTENT_TYPE_ATOM)
        except ResourceWarning:
    ...
    return frozenset(
        ecli
        for ecli in process_eclis(
            relevant_eclis=relevant_eclis,
...
def fetch_and_process_eclis(args):
    ...
    n_eclis_determined = int(
        number_of_eclis_xpath_regex.match(
            number_of_eclis_xpath(n_eclis_tree)[0]).groups(0)[0])
    ...
    rulings_ranges_indices = distribute_remainder_over_range_indices(
        0, n_eclis, n_requests_per_worker, remainder)

    xml_rulings_dir_path = args.xml_rulings_dir_path
    data_dir_path = xml_rulings_dir_path.joinpath('data')
    irrelevant_eclis_file_path = xml_rulings_dir_path.joinpath(
        args.irrelevant_eclis_file_name)
    ...
    if not xml_rulings_dir_path.is_dir():  # TODO: exists?
        ...
        if irrelevant_eclis_file_path.is_file(
        ) and irrelevant_eclis_file_path.stat().st_size != 0:
            with irrelevant_eclis_file_path.open(
                    mode='rb') as previous_irrelevant_eclis_file:
                ...
                if max_index > 0:
                    ...
                    futures.append(
                        pool.apply_async(
                            ...
                            args=(xml_rulings_dir_path, data_dir_path,
                                  complete_query, previous_irrelevant_eclis,
                                  log_level)))
