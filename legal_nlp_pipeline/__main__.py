...
def distribute_work(args):
    ...
    eclis = (determine_ecli(xml_ruling_file_path) for xml_ruling_file_path in xml_ruling_files_paths)

    node_distribution = tuple(chain.from_iterable((node['name'] for _ in range(node['weight'])) for node in cluster))
    ...
    for node_id, node_jobs in work_distribution.items():
        work_distribution_file_path = args.output_dir_path.joinpath(str(node_id)).with_suffix('.json')
        with work_distribution_file_path.open(mode='wt') as work_distribution_file:
            dump(node_jobs, work_distribution_file)
            info("Distributed {n_node_jobs:d} jobs to node {node_id}, as recorded at "
                 "'{work_distribution_file_path}'. ".
                 format(n_node_jobs=len(node_jobs),
                        node_id=node_id,
...
def pipeline(args):
    ...
    work_distribution_file_path = output_dir_path.joinpath(gethostname()).with_suffix('.json')

    try:
        with work_distribution_file_path.open(mode='rt') as work_distribution_file:
            work_distribution = load(work_distribution_file)
    except FileNotFoundError:
        warning("Work distribution not found at '{work_distribution_file_path}'. ".
    ...
    SubdirPaths = namedtuple('SubdirPaths', ('extracted_dir_path',
                                             'tokenized_dir_path',
                                             'selected_dir_path',
                                             'parsed_dir_path',
                                             'trees_dir_path',
                                             'postprocessed_dir_path',
                                             'classf_dir_path'))

    subdir_paths = SubdirPaths(extracted_dir_path=extracted_dir_path,
    ...
    for subdir_path in subdir_paths:
        if subdir_path.parent == output_dir_path:
            try:
                ...
                warning("Directory '{subdir_path}' already exists. ".format(subdir_path=subdir_path))
                pass
        else:
            info("Skipping '{subdir_path}' as its prefix '{prefix}' is not equal to '{temp_base_dir_path}', "
                 "so it is not rooted in the temporary base directory. ".
                 format(subdir_path=subdir_path,
...
if __name__ == '__main__':
    parser = ArgumentParser(prog='legal_nlp_pipeline', fromfile_prefix_chars='@')
    parser.add_argument('-v', default=INFO, type=int,
    ...
    fetch_xml_rulings_parser = subparsers.add_parser(fetch_and_process_eclis.__name__,
                                                     help='Fetch ECLIS and XML rulings relevant to query. ')
    fetch_xml_rulings_parser.set_defaults(func=fetch_and_process_eclis)
    fetch_xml_rulings_parser.add_argument('xml_rulings_dir_path', type=Path, help='XML rulings directory path. ')
    fetch_xml_rulings_parser.add_argument('--irrelevant_eclis_file_name', type=Path, default='irrelevant_eclis.pkl',
                                          help='Irrelevant ECLIs declaration file name. ')
    fetch_xml_rulings_parser.add_argument('--n_max_rulings', type=int, default=-1,
                                          help='Maximal number of rulings to fetch by ECLI (-1: all found). ')

    distribute_work_parser = subparsers.add_parser(distribute_work.__name__,
    ...
    distribute_work_parser.add_argument('output_dir_path', type=Path, help='Output directory path. ')
    distribute_work_parser.add_argument('xml_rulings_dir_path', type=Path, help='XML rulings directory path. ')
    distribute_work_parser.add_argument('-cluster_file_name', type=str, default='cluster.json',
                                        help='Cluster declaration file path. ')

    pipeline_parser = subparsers.add_parser(pipeline.__name__,
                                            help='Process ECLIs as node in cluster through pipeline according to work '
                                                 'distribution. ')
    pipeline_parser.set_defaults(func=pipeline)
    pipeline_parser.add_argument('output_dir_path', type=Path, help='Output directory path. ')
    pipeline_parser.add_argument('xml_rulings_dir_path', type=Path, help='XML rulings directory path. ')
    ...
    basicConfig(level=args.v, format='%(asctime)s - %(levelname)s - %(message)s')  # TODO: parameterize level
