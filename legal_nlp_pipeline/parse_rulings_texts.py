...
def wccount(file_path: Path):
    ...
    return int(Popen(('wc', '-l', str(file_path)),
                     stdout=PIPE,
...
def is_ready_to_parse(parsed_sentences_files_dir_path: Path):
    ...
    if not parsed_sentences_files_dir_path.exists():  # TODO: is_dire?
        ...
        if lock_file_path.exists():
            debug("Lock in place at {lock_file_path}. ".format(lock_file_path=lock_file_path))
        return False
...
def prepare_parsing(tokenized_file_path: Path, target_dir_path: Path):
    ...
    if tokenized_file_path.is_file() and tokenized_file_path.stat().st_size != 0:
        ...
        if is_ready_to_parse(parsed_sentences_files_dir_path):
            debug("Tokenized file at '{tokenized_file_path}' is going to be parsed. ".
            ...
            n_nonzero_parsed_sentence_files = len(nonzero_parsed_sentence_files)
            tokenized_file_line_count = wccount(tokenized_file_path)
            if n_nonzero_parsed_sentence_files == tokenized_file_line_count:
                info("Tokenized file at '{tokenized_file_path}' has already been parsed. ".
                     format(tokenized_file_path=tokenized_file_path))
                return None, None
                ...
                if not lock_file_path.exists():
                    warning("Removing incomplete ({n_nonzero_parsed_sentence_files:d} < {tokenized_file_line_count:d} "
                            "sentences) parsing in '{parsed_sentences_files_dir_path}'. ".
                            format(n_nonzero_parsed_sentence_files=n_nonzero_parsed_sentence_files,
                    ...
                    prepare_parsing(tokenized_file_path=tokenized_file_path, target_dir_path=target_dir_path)
                else:
                    return None, None
            elif n_nonzero_parsed_sentence_files > tokenized_file_line_count:
                raise RuntimeError('More nonzero-size sentences already parsed ({n_nonzero_parsed_sentence_files:d}) '
                                   ...
                                   'and current contents of "{tokenized_file_path}"? '.
                                   format(n_nonzero_parsed_sentence_files=n_nonzero_parsed_sentence_files,
                                          tokenized_file_line_count=tokenized_file_line_count,
                                          parsed_sentences_files_dir_path=parsed_sentences_files_dir_path,
                                          tokenized_file_path=tokenized_file_path))

        parsed_sentence_file_path_prefix = parsed_sentences_files_dir_path.joinpath(tokenized_file_path.name)

        return parsed_sentences_files_dir_path, parsed_sentence_file_path_prefix
    else:
        warning("No tokenized file recognized at '{tokenized_file_path}'. ".
                format(tokenized_file_path=tokenized_file_path))


def alpino_parse_tokenized_file_socket(tokenized_file_path: Path, target_dir_path: Path, host='127.0.0.1',
                                       base_port_number=42424, n_instances: int=4):
    """
        ...
        with tokenized_file_path.open(mode='rb') as tokenized_file:
            ...
            for sentence in tokenized_file.readlines():
                ...
                with socket(AF_INET, SOCK_STREAM) as alpino_socket:
                    ...
                    while True:
                        parsed_sentence_xml_chunk = alpino_socket.recv(ALPINO_SOCKET_BUFFER_SIZE)
                        if not parsed_sentence_xml_chunk:
                            alpino_socket.sendall(b'\n\n')
                            break
                        else:
                            parsed_sentence_buffer.write(parsed_sentence_xml_chunk)
                ...
                with parsed_sentence_file_path.open(mode='wb') as parsed_sentence_file:
...
def alpino_parse_tokenized_files_socket(tokenized_files_dir_path: Path, target_dir_path: Path, in_suffix: str,
                                        work_distribution,
                                        n_instances: int, host='127.0.0.1',
                                        base_port_number=42424):
    from logging import info

    tokenized_files = (tokenized_files_dir_path.joinpath(ecli).with_suffix(in_suffix) for ecli in
                       work_distribution)
    ...
    for tokenized_file_path in tokenized_files:
        if tokenized_file_path.is_file() and tokenized_file_path.stat().st_size != 0:
            alpino_parse_tokenized_file_socket(tokenized_file_path=tokenized_file_path, target_dir_path=target_dir_path)
            parsing_count += 1
        else:
            raise RuntimeError("Tokenized file '{text_file_path}' is not a file or has zero size. ".
                               format(text_file_path=tokenized_file_path))
...
def alpino_parse_tokenized_file_directly(tokenized_file_path: Path, target_dir_path: Path, cpu_core: str):
    ...
    parsed_sentences_files_dir_path, parsed_sentence_file_path_prefix = \
        prepare_parsing(tokenized_file_path=tokenized_file_path,
                        target_dir_path=target_dir_path)
    if parsed_sentences_files_dir_path is not None:
        command = ('taskset', '-a', '-c', cpu_core, 'Alpino', '-veryfast', '-notk',
...
def alpino_parse_tokenized_files_directly_multiprocessing(tokenized_dir_path: Path, target_dir_path: Path,
    ...
    tokenized_files = (tokenized_dir_path.joinpath(ecli).with_suffix(in_suffix) for ecli in work_distribution)
    ...
    with Pool(processes=n_cores) as pool:
        ...
        for tokenized_file_path in tokenized_files:
            if tokenized_file_path.is_file() and tokenized_file_path.stat().st_size != 0:
                futures.append(pool.apply_async(func=alpino_parse_tokenized_file_directly,
                                                args=(tokenized_file_path, target_dir_path, next(cpu_cores))))
            else:
                raise RuntimeError("Tokenized file at '{tokenized_file_path}' is not a file or has zero size. ".
                                   format(tokenized_file_path=tokenized_file_path))
    ...
    info("Performed parsings to '{target_dir_path}'. ".format(target_dir_path=target_dir_path))
